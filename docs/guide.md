...
- **可选模型辅助**：优先复用本机 DeepSeek Anthropic 兼容配置，默认使用 `deepseek-v4-flash`；没有 DeepSeek 配置时回退到本机 Codex Responses API。它只辅助识别本地规则无法解析的单条通知，模型结果进入人工确认。
...
- 识别模型优先读取 `~/.codex/config.toml` 中的 `ANTHROPIC_BASE_URL`、`ANTHROPIC_AUTH_TOKEN` 和 `ANTHROPIC_DEFAULT_HAIKU_MODEL`。可用环境变量 `CALENDAR_DEEPSEEK_BASE_URL`、`CALENDAR_DEEPSEEK_API_KEY`、`CALENDAR_DEEPSEEK_MODEL` 覆盖；密钥只从本机配置读取，不写入仓库。
